...
# Dev Services handles database automatically in development mode
...
%prod.quarkus.datasource.username=${DB_USER:parasol}
%prod.quarkus.datasource.password=${DB_PASSWORD:parasol}
%prod.quarkus.datasource.jdbc.url=jdbc:postgresql://${DB_HOST:localhost}:5432/${DB_NAME:parasol}

# Hibernate
quarkus.hibernate-orm.database.generation=update
----

IMPORTANT: Use the `%dev.` prefix for development-only settings and `%prod.` prefix for production-only settings. This allows Quarkus Dev Services to automatically configure PostgreSQL in development mode while using environment variables in production. Kafka Dev Services is disabled in development to use the external Kafka broker deployed via Helm (see xref:day2-lab1-supporting-services.adoc[Deploy Supporting Services]).
