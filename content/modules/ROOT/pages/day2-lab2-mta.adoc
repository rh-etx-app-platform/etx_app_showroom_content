...
IMPORTANT: Use the `%dev.` prefix for development-only settings and `%prod.` prefix for production-only settings. This allows Quarkus Dev Services to automatically configure PostgreSQL in development mode while using environment variables in production.
