...
Set your cluster credentials (from RHDP emails):
...
|1
|Developer
|Triggers promotion (manual PipelineRun)

|2
|Promotion Pipeline
|Resolves latest image tag from ImageStream (or uses override)

|3
|Promotion Pipeline
|Fetches Quay credentials from Vault

|4
|Promotion Pipeline
|Copies image from internal registry to Quay with 'stable' tag
...
==== Create Staging Database Secret

Create a `parasol-db-secret` in the staging namespace so the deployment can reference credentials without hardcoding them:

[source,bash,role=execute]
----
oc create secret generic parasol-db-secret \
  -n etx-app-staging \
  --from-literal=database-name=parasol \
  --from-literal=database-user=parasol \
  --from-literal=database-password=parasol
----
...
NOTE: This uses the same GitOps repository from the previous section. If you configured SSH authentication earlier, this push will work without prompts.
...
=== Resolve Promotion Candidate

Create a task that automatically determines which image to promote.
It queries the ImageStream for the most recent CI build, eliminating the need to manually look up commit SHAs.
If you need to promote a specific image (for example, during a rollback), you can pass an override tag.
...
spec:
  params:
    - name: override-tag
      type: string
      default: ""
      description: "Specific image tag to promote. Leave empty to use the latest CI build."
  results:
    - name: image-tag
      description: "Resolved image tag to promote"
  steps:
    - name: resolve
      ...
      script: |
        ...
        if [ -n "$(params.override-tag)" ]; then
          echo "Using override tag: $(params.override-tag)"
          echo -n "$(params.override-tag)" > $(results.image-tag.path)
        else
          echo "Querying ImageStream for latest CI build..."
          TAG=$(oc get imagestream etx-app-base-app -n etx-app-dev -o json | \
            jq -r '.status.tags
                   | map(select(.items != null and (.items | length > 0)))
                   | sort_by(.items[0].created)
                   | reverse
                   | .[0].tag // empty')

          if [ -z "$TAG" ]; then
            echo "ERROR: No images found in ImageStream. Run the CI pipeline first."
            exit 1
          fi

          echo "Resolved latest build: $TAG"
...
spec:
  params:
    - name: source-image-tag
      type: string
      default: ""
      description: "Image tag to promote. Leave empty to use the latest CI build."
    - name: dest-image-tag
      type: string
      default: stable
  ...
  tasks:
    ...
    - name: copy-to-quay
      runAfter: [resolve-image, fetch-quay-creds]
      taskRef:
        name: copy-image-to-quay
      params:
        - name: source-image
          value: image-registry.openshift-image-registry.svc:5000/etx-app-dev/etx-app-base-app:$(tasks.resolve-image.results.image-tag)
        - name: dest-image
...
The pipeline automatically resolves the latest image from the CI pipeline — no manual SHA lookup required.
...
NOTE: The `source-image-tag` parameter is omitted, so the pipeline uses the `resolve-promotion-candidate` task to automatically select the most recent image built by CI.

Watch the pipeline logs. You should see:

1. Image tag resolved from ImageStream
2. Vault credentials retrieved
3. Image copied from internal registry to Quay
4. GitOps repository updated

=== Promote a Specific Image (Rollback)

If you need to promote a specific image — for example, to rollback staging to a previous version — pass the SHA explicitly:
...
Then run the promotion with your chosen tag:
...
TIP: The default workflow (omitting `source-image-tag`) is ideal for promoting the latest successful build. Use the explicit SHA only for rollbacks or when promoting a specific tested version.
...
=== Method 2: Re-promote a Previous Image

Run the promotion pipeline with a specific previous image tag:

[source,bash,role=execute]
----
# List available images to find the previous SHA
oc get imagestream etx-app-base-app -n etx-app-dev -o json | \
  jq -r '.status.tags[].tag' | head -5
----

Then promote the desired version:
...
* Automatic image resolution from CI builds (no manual SHA lookup)
...
* Rollback capability through Git history and explicit SHA promotion
...
* The pipeline auto-resolves the latest CI build — no manual lookup needed
* Override with a specific SHA for rollbacks or controlled releases
* This pattern balances automation with human control
