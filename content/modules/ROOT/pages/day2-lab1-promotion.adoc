...
oc get imagestream parasol-insurance -n etx-app-dev -o json | \
...
tkn pipeline start parasol-promotion \
  ...
  --param gitops-repo-url={gitlab_url}/software-factory/etx-app-gitops.git \
...
[source,bash,role=execute]
----
oc get application parasol-stage -n etx-app-dev -o jsonpath='{.status.sync.status}'
----
