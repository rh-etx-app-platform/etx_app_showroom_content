...
First, get an actual image tag from the CI pipeline builds:

[source,bash,role=execute]
----
IMAGE_TAG=$(oc get imagestream etx-app-base-app -n etx-app-dev -o jsonpath='{.status.tags[0].tag}')
echo "Image tag to promote: $IMAGE_TAG"
----

Then create a PipelineRun using this tag:
