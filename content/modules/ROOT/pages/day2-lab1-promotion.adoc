...
Quay authenticates interactive users through Keycloak SSO, so the bootstrap `quayadmin` credentials can no longer be used for automated pushes.
You will create the organization and repository the pipeline expects, create a robot account for non-interactive authentication, and update Vault with the robot credentials.
...
. Configure the repository:
  ...
  * **Repository Visibility:** Public (so the staging cluster can pull without an image pull secret)
...
=== Update Vault with Robot Credentials

Replace the bootstrap `quayadmin` values in Vault with the robot account credentials so the promotion pipeline can authenticate to Quay.
...
Next, get the Vault token for login
[source,bash,role=execute]
----
...
. Open link:{vault_url}
. Log in with the *OIDC* method using `platform-admin` / `openshift`
. From the `Dashboard` click the btn:[View] button next to `secret/` under *Secret engines*
. Navigate to `secret/registry/quay`
. Click the *Secret* tab
. Click btn:[Create new version] (or edit the existing secret)
...
TIP: Leave any existing `server` value unchanged if it already matches the Quay route hostname. Only the `username` and `password` fields must change from `quayadmin` to the robot account.
