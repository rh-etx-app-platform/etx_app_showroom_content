...
. In the `etx-app` organization, open the *Robot Accounts* tab
. Click btn:[Create Robot Account]
. Name the robot `promoter` and click btn:[Create Robot Account]
. Grant the robot *Write* permission on the `etx-app-base-app` repository
. When Quay displays the robot credentials, copy both values:
  * **Username:** `etx-app+promoter`
  * **Token:** the generated token

IMPORTANT: Save the robot token now. Quay will not show it again after you close the dialog.
