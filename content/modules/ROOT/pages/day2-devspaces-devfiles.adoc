...
Commands define tasks available in the IDE's command palette:
...
Recommend VS Code extensions by adding an `extensions.json` attribute.
Dev Spaces installs these automatically when the workspace starts:
...
Proceed to xref:day2-devspaces-workspace.adoc[Create a Workspace] to put it all together with nested containers and Quarkus Dev Services.
