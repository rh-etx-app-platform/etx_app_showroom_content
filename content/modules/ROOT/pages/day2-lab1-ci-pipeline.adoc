...
NOTE: The `fetch-creds` task demonstrates Vault integration for retrieving registry credentials. For the OpenShift internal registry, the pipeline ServiceAccount authenticates automatically via its token. The Vault-fetched credentials become essential in the xref:day2-lab1-promotion.adoc[Promotion Pipeline] where images are pushed to an external registry (Quay).
