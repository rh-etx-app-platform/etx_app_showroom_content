...
. Log into GitLab as administrator:
  * Open the GitLab URL in your browser
  * *Username:* `platform-admin`
  * *Password:* `openshift`
  * Click *ETX Lab SSO* button to authenticate via Keycloak
...
The Admin Area is only accessible to users with administrator privileges. When logged in as `platform-admin`, you should see:
...
* You're logged in as `platform-admin` (not `platform-developer`)
* You clicked "ETX Lab SSO" during login
* Your user has admin role in GitLab
...
You need Maintainer or Owner role to configure webhooks. Use `platform-admin` account or request access.
