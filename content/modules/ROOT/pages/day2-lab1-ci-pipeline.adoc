...
----
GITLAB_URL="https://$(oc get route -n gitlab -l app=webservice -o jsonpath='{.items[0].spec.host}')"
echo "GitLab Admin URL: $GITLAB_URL/admin"
----
+
...
. Navigate to menu:Admin Area[Settings > Network]
...
[TIP]
====
You can verify this setting is enabled by looking for a green checkmark next to "Allow requests to the local network" in the Network settings page.
====
