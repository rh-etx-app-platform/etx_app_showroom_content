...
----
+
. Create a new project: *etx-app-gitops*
. Clone it locally:
+
...
oc get application parasol-dev -n etx-app-dev \
...
[source,bash,role=execute]
----
oc get pods -n etx-app-dev -l app=parasol-insurance
...
[source,bash,role=execute]
----
oc get pod -n etx-app-dev -l app=parasol-insurance \
  ...
  steps:
    - name: update-image
      ...
      script: |
        ...
        sed -i "s|image:.*parasol-insurance:.*|image: image-registry.openshift-image-registry.svc:5000/etx-app-dev/etx-app-base-app:$(params.image-tag)|" deployment.yaml
...
argocd login $(oc get route argocd-server -n etx-app-dev -o jsonpath='{.spec.host}') \
  --insecure --username admin --password $(oc get secret argocd-cluster -n etx-app-dev -o jsonpath='{.data.admin\.password}' | base64 -d)
...
Update `environments/stage/deployment.yaml` to use the `etx-app-staging` namespace:
        ...
        - name: etx-app-base-app
          ...
          resources:
            ...
            limits:
              ...
              memory: 2Gi
----
<1> Update namespace to etx-app-staging
...
Update `environments/stage/service.yaml` and `environments/stage/route.yaml` to use `etx-app-staging` namespace.
...
[source,bash,role=execute]
----
oc get application parasol-stage -n etx-app-dev \
  -o jsonpath='{.status.sync.status}'
----
...
oc get application parasol-dev -n etx-app-dev -o jsonpath='{.status.sync.status}: {.status.health.status}'

# Check application pods
oc get pods -n etx-app-dev -l app.kubernetes.io/name=parasol-app

# Check application health endpoint
curl http://$(oc get route parasol-app -n etx-app-dev -o jsonpath='{.spec.host}')/q/health

# Check OpenTelemetry injection
oc get pod -n etx-app-dev -l app.kubernetes.io/name=parasol-app \
  -o jsonpath='{.items[0].spec.initContainers[*].name}'
----
