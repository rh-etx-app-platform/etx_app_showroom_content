...
. Create a new blank and public project: *etx-app-gitops*
. Uncheck any option from the "Project Configuration" section.
. Clone it locally:
+
...
EOF
----
<1> The `etx-app-dev` namespace is created from `environments/dev/namespace.yaml` when Argo CD syncs
...
Create `environments/stage/deployment.yaml`:
...
Create `environments/stage/service.yaml`:

[source,bash,role=execute]
----
cat > environments/stage/service.yaml <<'EOF'
apiVersion: v1
kind: Service
metadata:
  name: etx-app-base-app
...
Create `environments/stage/route.yaml`:

[source,bash,role=execute]
----
cat > environments/stage/route.yaml <<'EOF'
apiVersion: route.openshift.io/v1
kind: Route
metadata:
  name: etx-app-base-app
