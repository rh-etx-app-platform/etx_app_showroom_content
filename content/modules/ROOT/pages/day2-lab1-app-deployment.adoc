...
Update the ArgoCD ConfigMap to enable Keycloak OIDC authentication using the same configuration as etx-gitops:
...
# Update ArgoCD ConfigMap with ETX Keycloak OIDC configuration
oc patch configmap argocd-cm -n etx-app-dev --type=merge -p "{\"data\":{
  \"url\": \"${ARGOCD_URL}\",
  \"oidc.config\": \"name: ETX Keycloak\\nissuer: ${KEYCLOAK_URL}/realms/etx-lab\\nclientID: argocd\\nclientSecret: \\\$argocd-oidc-secret:clientSecret\\nrequestedScopes:\\n  - openid\\nrequestedIDTokenClaims:\\n  groups:\\n    essential: true\\nlogoutURL: ${KEYCLOAK_URL}/realms/etx-lab/protocol/openid-connect/logout\"
}}"

# Update RBAC policy to match etx-gitops configuration
oc patch configmap argocd-rbac-cm -n etx-app-dev --type=merge -p '{
  "data":{
    "policy.csv": "g, platform-admin, role:admin\ng, platform-developer, role:admin\ng, argocd-admins, role:admin",
...
echo "Configuration updated to match etx-gitops integration with ETX Keycloak"
----
...
. Click *LOG IN VIA ETX KEYCLOAK*
. You'll be redirected to ETX Keycloak
