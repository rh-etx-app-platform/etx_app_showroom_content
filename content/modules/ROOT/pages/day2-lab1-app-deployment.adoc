...
# The shared argocd OIDC client secret (pre-configured by the ETX bootstrap)
...
IMPORTANT: The ArgoCD OIDC client is shared across all ArgoCD instances in the ETX platform. Do not create a new client in Keycloak. Use the existing `argocd` client with the provided secret. The value `changeme` is the actual pre-configured client secret from the ETX bootstrap -- do not change it unless your instructor provides a different value.
