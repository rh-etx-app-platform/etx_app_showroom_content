...
[source,bash,role=execute]
----
oc apply -f etx-app-dev-app.yaml
----
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: etx-app-base-app
          image: {quay_url}/etx-app/etx-app-base-app:stable # <3>
          ports:
            - containerPort: 8080
          resources:
            requests:
...
[source,bash,role=execute]
----
oc apply -f etx-app-stage-app.yaml
----
...
oc get pods -n etx-app-dev -l app.kubernetes.io/name=etx-app-base-app

# Check application health endpoint
curl http://$(oc get route etx-app-base-app -n etx-app-dev -o jsonpath='{.spec.host}')/q/health

# Check OpenTelemetry injection
oc get pod -n etx-app-dev -l app.kubernetes.io/name=etx-app-base-app \
  -o jsonpath='{.items[0].spec.initContainers[*].name}'
----
