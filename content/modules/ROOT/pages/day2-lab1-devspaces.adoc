...
# Store your GitLab credentials
# Replace YOUR_PERSONAL_ACCESS_TOKEN with the token you copied
cat > ~/.git-credentials <<EOF
https://platform-developer:YOUR_PERSONAL_ACCESS_TOKEN@gitlab.{deployer_domain}
EOF
...
IMPORTANT: Replace `YOUR_PERSONAL_ACCESS_TOKEN` in the command above with the actual token you copied from GitLab.

The credentials are now stored in `~/.git-credentials` and Git will use them automatically for all future operations.
...
If the push succeeds without prompting for credentials, your Git authentication is configured correctly.
