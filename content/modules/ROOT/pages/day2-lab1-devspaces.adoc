...
GitLab is configured with Keycloak OIDC authentication. While Dev Spaces can clone repositories using OAuth, **push operations require a Personal Access Token (PAT)** because GitLab OIDC does not support username/password authentication for Git operations.
...
==== Store Credentials in Dev Spaces

Configure Git to cache your credentials in the Dev Spaces terminal:
...
The next time you perform a `git push`, Git will prompt for credentials in the terminal. When that happens:

* At `Username:` prompt → type `platform-developer` and press Enter
* At `Password:` prompt → paste your Personal Access Token and press Enter

The credentials will be automatically saved to `~/.git-credentials` for all future git operations.
...
When you run `git push` for the first time, you'll see prompts in the terminal:

[source,text]
----
Username for 'https://gitlab.apps.cluster-GUID.dyn.redhatworkshops.io': platform-developer
Password for 'https://platform-developer@gitlab.apps.cluster-GUID.dyn.redhatworkshops.io':
----

Type your username, press Enter, paste your PAT token, press Enter.

Expected output after entering credentials:

[source,text]
----
Enumerating objects: 5, done.
...
To https://gitlab.apps.cluster-GUID.dyn.redhatworkshops.io/software-factory/etx_app_base_app.git
   abc1234..def5678  feature/add-devfile -> feature/add-devfile
----

If the push succeeds, your Git credentials are configured correctly and cached for future operations.
