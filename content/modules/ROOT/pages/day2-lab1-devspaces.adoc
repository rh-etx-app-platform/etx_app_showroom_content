...
== Step 2: Configure Git and Create Feature Branch

Once the workspace opens, configure Git and create a feature branch for the devfile changes.

Open a terminal from the main menu (hamburger icon in the top-left corner: menu:Terminal[New Terminal]).

Configure Git identity and create a feature branch:
...
projects:
  - name: etx-app-base-app
    git:
      remotes:
        origin: "$GITLAB_URL/software-factory/etx_app_base_app.git"
      checkoutFrom:
        revision: feature/add-devfile
components:
...
Add and commit the devfile to the feature branch:
...
# Print the URL to use for workspace creation
echo ""
echo "Use this URL to create your workspace in Step 5:"
echo "$GITLAB_URL/software-factory/etx_app_base_app.git#feature/add-devfile"
echo ""
----
...
IMPORTANT: Copy the URL printed at the end of the commands above. You will need it in Step 5.

== Step 5: Create Workspace from Devfile
...
. Click btn:[Create Workspace]
. Select *Import from Git*
. Paste the repository URL that was printed at the end of Step 4
+
It should look like:
+
[source,text]
----
https://gitlab.apps.cluster-xxxxx.dynamic2.redhatworkshops.io/software-factory/etx_app_base_app.git#feature/add-devfile
----
. Click btn:[Create & Open]
